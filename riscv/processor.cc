...
void processor_t::take_interrupt(reg_t pending_interrupts)
{
  ...
  //if (!pending_interrupts) { //CHATH - disabling all interrupts for now
    return;
  //}
...
void processor_t::take_trap(trap_t& t, reg_t epc)
{
  ...
  if (state.prv <= PRV_S && bit < max_xlen && ((vsdeleg >> bit) & 1)) {
    ...
    state.mcause->write(t.cause() & 15);
    if (t.cause() != CAUSE_ILLEGAL_INSTRUCTION){ //CHATH -proFuzz: To match BOOM behaviour
...
void processor_t::disasm(insn_t insn)
{
  ...
  if (last_pc != state.pc || last_bits != bits) {
    ...
    s << "core " << std::dec << std::setfill(' ') << std::setw(3) << id
      ...
      << zext(state.pc, max_xlen) << " (0x" << std::setw(8) << bits << ") [0x" << state.mstatus->read() << "," << std::setw(1) << state.frm->read() << "," << std::setw(2) << state.fflags->read() << "," << std::setw(2) << state.mcause->read() 
	<< "," << std::setw(2) << state.scause->read()
	<< "," << std::setw(8) << state.medeleg->read() //<< "," << std::setw(8) << state.csrmap[CSR_SEDELEG]->read()
	<< "," << std::setw(8) << state.mcounteren->read()
	<< "," << std::setw(8) << state.scounteren->read() //	<< "," << std::setw(4) << state.csrmap[CSR_PMPCFG0]->read()
	<< "," << std::setw(8) << state.dcsr->read()
